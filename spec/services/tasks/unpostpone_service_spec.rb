...
RSpec.describe Tasks::UnpostponeService do
  ...
  before do
    ...
    allow(TaskRepository).to receive(:update).with(postponed_task.id, default_params)
  end
  ...
  describe '#call' do
    it 'sets default params correctly' do
      ...
      expect(TaskRepository).to have_received(:update).with(postponed_task.id, default_params)
    end
  end
end
