...
module Api
  module V1
    class TasksController < BaseController
      ...
      def index
        tasks = TaskRepository.by_user_id(current_user.id)
      ...
      def create
        ...
        if result.success?
          task = Tasks::CreateService.call(result.to_h.merge(user_id: current_user.id))
      ...
      def task
        @task ||= TaskRepository.by_id(params[:id])
      end
    end
  end
end
