...
module Api
  module V1
    class WishesController < BaseController
      ...
      def index
        wishes = WishRepository.by_user_id(current_user.id)
      ...
      def create
        result = validate_params(contract: Wishes::CreateContract.new, params: params[:wish])

        if result.success?
          task = Wishes::CreateService.call(result.to_h.merge(user_id: current_user.id))
      ...
      def wish
        @wish ||= WishRepository.by_id(params[:id])
      end
    end
  end
end
